...
async function getPopularGamesFromApi() {
  let popularGames = [];
  try {
    ...
    getSearchPopularGamesFromApi(popularGames)
  }
}

async function getSearchPopularGamesFromApi(popularGames) {
  let queries = ""
  popularGames.forEach(element => {
    queries = queries + `query games "Popular ${element.game_id}" { fields name; where id = ${element.game_id}; };`
  });
  console.log("popular games", popularGames, queries)
  try {
    const { data } = await useFetch('/api/igdb-api', {
      method: 'POST',
